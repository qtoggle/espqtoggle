...
ICACHE_FLASH_ATTR static void   configure_duty(port_t *port, bool enabled);
ICACHE_FLASH_ATTR static double read_value_duty(port_t *port);
ICACHE_FLASH_ATTR static bool   write_value_duty(port_t *port, double value);

ICACHE_FLASH_ATTR static void   configure_freq(port_t *port, bool enabled);
ICACHE_FLASH_ATTR static double read_value_freq(port_t *port);
ICACHE_FLASH_ATTR static bool   write_value_freq(port_t *port, double value);

ICACHE_FLASH_ATTR static void   init(peripheral_t *peripheral);
ICACHE_FLASH_ATTR static void   cleanup(peripheral_t *peripheral);
...
void init(peripheral_t *peripheral) {
    ...
    used_gpio_mask |= 1L << user_data->pin;
...
void make_ports(peripheral_t *peripheral, port_t **ports, uint8 *ports_len) {
    ...
    if (!freq_port_created) {
        ...
        port_t *freq_port = zalloc(sizeof(port_t));
